...
class OthelloDensenet(nn.Module):
    def __init__(self, game, dropout=0.3):
        ...
        self.fc1 = nn.Linear(512*self.board_x*self.board_y, 1024, bias=True)
        self.fc_bn1 = nn.BatchNorm1d(1024)
        self.fc2 = nn.Linear(1024, 512, bias=True)
        self.fc_bn2 = nn.BatchNorm1d(512)
        self.fc3 = nn.Linear(512, self.action_size, bias=True)
    ...
    def forward(self, x):
        ...
        x = self.fc_bn0(x)
        x = x.view(-1, 512*self.board_x*self.board_y)
        x = F.dropout(F.relu(self.fc_bn1(self.fc1(x))),
                      p=self.dropout, training=self.training)
        x = F.dropout(F.relu(self.fc_bn2(self.fc2(x))),
                      p=self.dropout, training=self.training)
